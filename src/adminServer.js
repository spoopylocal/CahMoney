        ...
        <form id="editor" class="hidden" onsubmit="saveUser(event)">
          <div class="panel">
            ...
            <div class="grid-2">
              ...
              <div><label>Rig JSON</label><input id="rig" placeholder="null"></div>
            </div>
            <div class="grid-3">
...
<script>
const numberFields = ["wallet","bank","experience","lastBeg","lastWork","lastDaily","lastRob","lastBankrob","lastHunt","lastGive","lastMine"];
let users = [];
let items = [];
let boosts = [];
let selected = null;
...
function showEditor(userId, user) {
  ...
  document.getElementById("rig").value = JSON.stringify(user.rig ?? null);
...
function collectUser() {
  ...
  user.rig = JSON.parse(document.getElementById("rig").value || "null");
  return user;
}
